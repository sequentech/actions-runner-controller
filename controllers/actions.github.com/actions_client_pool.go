...
func (r *k8sResolver) appConfig(ctx context.Context, key string) (*appconfig.AppConfig, error) {
	...
	if err := r.client.Get(
		ctx,
		types.NamespacedName{Namespace: r.namespace, Name: key},
		secret,
	); err != nil {
		return nil, fmt.Errorf("failed to get secret %q", key)
	}
...
func (r *k8sResolver) proxyCredentials(ctx context.Context, key string) (*url.Userinfo, error) {
	...
	return url.UserPassword(string(secret.Data["username"]), string(
